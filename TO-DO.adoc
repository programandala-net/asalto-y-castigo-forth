...
Hacer `setup-entity` innecesario, para que no haya que salvar y
restaurar campos durante la inicialización de los entes. El campo que
guarda el xt de inicialización podría salvarse muy fácilmente: basta
con definirlo en la primera posición de la estructura, y tenerlo en
cuenta al hacer `erase`.
