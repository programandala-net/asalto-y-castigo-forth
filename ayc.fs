...
vocabulary menu_vocabulary  \ palabras del menú \ XXX TODO -- not used yet
...
variable ignore_unknown_words?  \ ¿Ignorar las palabras desconocidas?  \ XXX TODO -- not used yet
...
variable background_paper  \ XXX TMP -- experimental
...
: init_colors  ( -- )
  ...
  [defined] background_paper [if]
    black background_paper !  \ XXX TMP -- experimental
  [then]
  gray about_ink !
  black about_paper !
...
: $two_main_exits_in_cave ( ca1 len1 ca2 len2 -- ca3 len3 )
...
: 1-exit_cave_description   ( a1 -- ca len )
...
soldiers% :attributes
  ...
  self% familiar++
  self% is_decoration
  \ self% has_definite_article  \ XXX TODO -- mejor implementar que tenga posesivo...
  self% belongs_to_protagonist  \ XXX TODO -- ...aunque quizá esto baste
  ;attributes
...
ravine_wall% :attributes
  ...
  self% is_not_listed  \ XXX OLD -- innecesario
  self% is_decoration
  ;attributes
...
variable silent_well_done?  \ XXX TODO -- not used yet
...
: exit>list  ( u -- )
  ...
  [debug_do_exits] [if]  cr ." exit>list" cr .stack  [then]  \ XXX INFORMER
  exit_separator$ »+
...
: free_exits  ( a -- u )
  ...
  [debug_do_exits] [if]  cr ." free_exits" cr .stack  [then]  \ XXX INFORMER
  0 swap
...
:action do_take
  ...
  ;action
: >do_drop_done_v1$  ( a -- ca1 len1 ) { object }
...
:action do_hit
  ...
  \ s" golpear"  main_complement+is_nonsense \ XXX TMP
  ;action
...
: toward_that_direction  ( a -- ca2 len2 )
...
: do_go_if_possible  ( a -- )
  ...
  [debug] [if]  s" Al entrar en DO_GO_IF_POSSIBLE" debug  [then]  \ XXX INFORMER
...
:action do_go
  ...
  then
  [debug] [if]  s" Al salir de DO_GO" debug  [then]  \ XXX INFORMER
  ;action
:action do_go_north
...
:action do_go_south
  \ Acción de ir al sur.
  [debug_catch] [if]  s" Al entrar en DO_GO_SOUTH" debug  [then]  \ XXX INFORMER
  tool_complement{unnecessary}
  south% main_complement{this_only}
  south% do_go_if_possible
  [debug_catch] [if]  s" Al salir de DO_GO_SOUTH" debug  [then]  \ XXX INFORMER
  ;action
:action do_go_east
...
: do_speak_if_possible  ( a -- )
  ...
  [debug] [if]  s" En DO_SPEAK_IF_POSSIBLE" debug  [then]  \ XXX INFORMER
  case
...
preposition: «sin»_preposition_bit «sin»_preposition#  \ XXX NOTE: para dejar cosas antes de la acción
[then]
( n )
...
: valid_parsing?  ( ca len -- wf )
  ...
  \ [debug_catch] [if]  s" En VALID_PARSING? antes de preparar CATCH" debug  [then]  \ XXX INFORMER
  [debug_parsing] [??] ~~
...
\ Prestos
' s" alias s"
: presto_de_pantalla_llena  ( ca len -- )  scroll_prompt place  ;
: presto_de_pausa_de_narración  ( ca len -- )  narration_prompt place  ;
: presto_de_fin_de_escena  ( ca len -- )  scene_prompt place  ;
: presto_de_comando  ( ca len -- )  command_prompt place  ;
...
: tomar  ['] do_take|do_eat action!  ; \ XXX TODO -- unfinished
' tomar  synonyms{
